...
**Session 2026-08-20 Phase 3 kickoff (Wave 3.1 — foundation + TUI live refresh landed):** ETW dependency verified: `ferrisetw 1.2.0` transitively pulls `windows 0.57.0` (separate from the project's locked 0.62 — double windows-rs + type friction, Pitfall #10). **Decision: implement the ETW change-trigger directly on the existing windows 0.62 ETW API** (added `Win32_System_Diagnostics_Etw` feature) — trigger-only design needs no payload parsing (ground truth = `GetExtendedTcpTable`, PITFALL #15). Keep windows 0.62 (STACK.md's 0.73 is stale; no Phase 3 requirement to migrate). Landed: `events.rs` (EventBus/CoreEvent, tested) + `monitor.rs`/`monitor/poller.rs` (2s PollTick, tested) — `8bda6df`; TUI live refresh wired — `0e148fb`: run_event_loop subscribes, PollTick → rescan via existing guard, 5s AUTO_REFRESH timer + `last_auto_refresh` removed, status bar `Live (polling)`/`Live (ETW)` via new `LiveMode` enum. Full state: 92 core + 17 TUI + integration green (kill_integration 2 flaky tests remain the documented pre-existing wait_for_exit harness issue).

**Next action:** implement `monitor/etw.rs` — requires (a) doc-grounded verification of the enable-provider ControlCode value (the 0.62 binding names it `ENABLECALLBACK_ENABLED_STATE(1)`, not a clear `EVENT_TRACE_CONTROL_ENABLE_PROVIDER` constant) and EVENT_TRACE_PROPERTIES raw-layout handling, and (b) a REAL Windows ETW session to validate (orphan-cleanup PITFALL #5, realtime mode) — a Phase 3 manual-UAT surface. On landing, publish `CoreEvent::LiveMode{etw:true}` + `NetworkChanged`; the poller then auto-degrades to fallback cadence.
